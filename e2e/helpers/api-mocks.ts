...
export async function mockSearchAPIs(page: Page) {
  ...
  await page.route(url => url.pathname.match(/\/api\/v1\/job-description-searches\/.*\/name/), async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
  ...
  await page.route(url => url.pathname.match(/\/api\/v1\/candidate-generation\/job-description-searches\/.*\/limit\/.*/), async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
